...
--2.
---------
INSERT INTO offence VALUES (52, 'Speeding', 169, '2022-05-15 18:32:00.000', 614223004, 1819887, 5531206, 'Rainy', 'Bright','Stormy','MKL090', 52)
...
VALUES
(52, 'Shifaz', 'Ali', '23-08-1998', 'Male', '0214455543', 'Gate 1 Knighton Road', 'Hamilton', 'Waikato', 'KA111112')
...
delete from offence where offenceId = 52
-------
update offence set description = 'Speeding' where offenceId = 1
update offence set speedAlleged = 130 where offenceId = 1
...
select * from vehicle
--seprate query for speed limit, speed alleged, exceeded by(could make just this seprate?) in the textbox

select offenderId, fname, lname from offender

select fname, lname from offence ofnce join offender ofnder on ofnce.offenderId = ofnder.offenderId join infringementNotice inf on inf.offenceID = ofnder.offenderId  where ofnder.offenderId = 1 --offenderid variable 

--could specify which infringement notice with fname and lname... maybe offenderID too
select * from offence ofnce join offender ofnder on ofnce.offenderId = ofnder.offenderId join infringementNotice inf on inf.offenceID = ofnder.offenderId join [location] as loc on loc.locationX = ofnce.locationX 

select offenderId, fname, lname from offender o join infringementNotice i on o.offenderId = i.offenceID
-------------------------
select DATEADD(DD, 2, issueDate) from infringementNotice

----------------------------

select ofnder.offenderId, fname, lname, streetAddress, cityAddress, regionAddress, dateOfBirth, licenseNumber, noticeNumber, icn, issueDate, ofnce.[dateTime], veh.make, veh.registrationNumber, loc.location, loc.road, loc.region,[description], amount, DATEADD(DD, 16, issueDate) as [Fee Due Date], speedLimit, speedAlleged from offence ofnce join offender ofnder on ofnce.offenderId = ofnder.offenderId join infringementNotice inf on inf.offenceID = ofnder.offenderId join [location] loc on loc.locationX = ofnce.locationX join vehicle veh on veh.registrationNumber = ofnce.registrationNumber where ofnder.offenderId = 11
--Add another 28 days to the Fee Due date to show its exceeded?

-------------------------------------
--4.
--All without infringement notices
select fname, lname, [description], speedAlleged, [dateTime], offend.offenderId, registrationNumber from offence ofnce join offender offend on ofnce.offenderId = offend.offenderId where offend.offenderId in (select offenderId from offender where offenderId not in (select offenceID from infringementNotice))

--unpaid infringements
select ofnder.offenderId, fname, lname, [description], amount, issueDate, noticeNumber  from infringementNotice inf join offender ofnder on inf.offenceID = ofnder.offenderId join offence ofnce on ofnce.offenceId = inf.offenceID where status like 'Unpaid'

--overdue infringements
select [dateTime], issueDate from offence ofnce join infringementNotice inf on ofnce.offenceId = inf.offenceID

--This one used below:(modified)
select ofnder.offenderId, fname, lname, [description],  amount,issueDate, dateTime, phone, noticeNumber from infringementNotice inf join offence ofnce on inf.offenceID = ofnce.offenceId join offender ofnder on ofnder.offenderId = ofnce.offenceId where DATEDIFF(DAY, issueDate, GETDATE()) > 28 and status like 'Unpaid'

--orignial
select ofnder.offenderId, fname, lname, [description],  amount,issueDate, dateTime, phone, noticeNumber from infringementNotice inf join offence ofnce on inf.offenceID = ofnce.offenceId join offender ofnder on ofnder.offenderId = ofnce.offenceId where DATEDIFF(DAY, dateTime, issueDate) > 28 and status like 'Unpaid'

select * from infringementNotice

INSERT INTO offender (offenderId, fname, lname, dateOfBirth, gender, phone, streetaddress, cityaddress, regionaddress, licenseNumber)
VALUES (51, 'Jane', 'Doe', '02-09-1998', 'Female', '0214434543', 'Gate 123 Knight Road', 'Hamilton', 'Waikato', 'KA111112')


INSERT INTO offence (offenceID, description, speedAlleged, dateTime, icn, locationX, locationY, rainConditions, lightConditions, windConditions, offenderId, registrationNumber)
VALUES (51, 'Speeding', 120, '1-07-2022 12:07:00', 218213064, 1819887, 5531206, 'Rainy', 'Bright', null, 51 ,'GLG909')

INSERT INTO infringementNotice (noticeNumber, amount, status, issueDate, offenceID)
VALUES ('PC0000051', 66.7, 'Unpaid', '03-15-2022', 51)

delete from infringementNotice where offenceID = 51
select * from offence
select * from infringementNotice
select * from offender
...
select COUNT(offenderId) as [Number of Recorded Offences] from offence

--sum of infringement fees
select SUM(amount) as [Sum of Infringement Fees] from infringementNotice

--average of how much the speed limit was exceeded by
select AVG(speedAlleged - speedLimit) as [Speed Limit Exceeded by] from offence ofnce join [location] loc on ofnce.locationX = loc.locationX 

--days the most offences occured on
select datename(dw,dateTime), COUNT(datename(dw,dateTime))  from offence group by datename(dw,dateTime) order by COUNT(datename(dw,dateTime)) desc 

--good to know i guess...
select CONVERT(date, datetime) , COUNT(CONVERT(date, datetime)) as occurs from offence group by CONVERT(date,dateTime) having COUNT(CONVERT(date, datetime)) > 1
-------------------------------------
--6
--Write the sql queries
-- Use select queries and store the outputs in variables in c#
-- put the vars in arrays and graph it

select * from offence
select * from location

select offenceId, roadLane, speedLimit, lightConditions from offence ofnce join location loc on ofnce.locationX = loc.locationX
--number of lanes vs number of offences
select roadLane, COUNT(roadLane) as [Number of offences] from offence ofnce join location loc on ofnce.locationX = loc.locationX group by roadLane

-- speed lim vs num of offences
select speedLimit, COUNT(speedLimit) as [Number of offences at speedlimit] from offence ofnce join location loc on ofnce.locationX = loc.locationX group by speedLimit
 

 --light conditions vs num of offences
 select lightConditions, COUNT(lightConditions) as [Number of offences at lightConditions] from offence ofnce join location loc on ofnce.locationX = loc.locationX group by lightConditions



 --for days vs num offences
 select datename(dw,dateTime) as Day, COUNT(datename(dw,dateTime)) as [values]  from offence group by datename(dw,dateTime) ORDER BY 
     CASE  datename(dw,dateTime)
          WHEN 'Sunday' THEN 1
          WHEN 'Monday' THEN 2
          WHEN  'Tuesday' THEN 3
 ...
 --hours vs num offences
 select datename(HH,dateTime) as Hour, COUNT(datename(dw,dateTime)) as offnces  from offence group by datename(HH,dateTime) order by Hour asc

 --age vs num offences
 select * from offence
 select DATEDIFF(YEAR, dateOfBirth, GETDATE()) as [Age], COUNT(dateTime) as [Offences Committed] from offender ofnder join offence ofnce on ofnce.offenderId = ofnder.offenderId group by DATEDIFF(YEAR, dateOfBirth, GETDATE()) 

 select fname, lname, DATEDIFF(YEAR, dateOfBirth, GETDATE()) as [Age] from offender

 --color veh vs num offences
 select colour as [Color of the car], COUNT(colour) as [Number of Offences] from vehicle veh join offence ofnce on ofnce.registrationNumber = veh.registrationNumber group by colour
-------------------------------------
